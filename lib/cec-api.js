...
exports.create = ({ config }) => {
  ...
  cec.on('REPORT_POWER_STATUS', function (packet, status) {
    ...
    for (var i = keys.length - 1; i >= 0; i--) {
      if (CEC.PowerStatus[keys[i]] == status) {
        ...
        evt.emit(deviceId + '.power', powerStatus)
        break;
      }
    }
  });
  ...
  return {
    ...
    setActiveSource(device) {
      ...
      const byte2 = details.physicalAddressBytes.length > 1 ? details.physicalAddressBytes[1] : undefined;
