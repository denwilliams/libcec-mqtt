...
exports.create = ({ config, cecApi }) => {
  ...
  DEVICE_IDS.forEach(id => {
    ["power", "physicaladdress", "name", "active"].forEach(name => {
      cecApi.on(`${id}.${name}`, data => service.emit(`${id}/${name}`, data));
    });
  });

  cecApi.on("active", data => service.emit("active", data));

  function powerDevice(deviceId, isOn) {
    if (isOn) {
  ...
  service.start = () => {
    ...
    mqttClient.on("connect", () => {
      ...
      DEVICE_IDS.forEach(id => {
        subscribe(
          `${topicPrefix}/device/${id}/input/power`,
          powerDevice.bind(null, id)
        );
        subscribe(
          `${topicPrefix}/device/${id}/input/on`,
          turnDeviceOn.bind(null, id)
        );
        subscribe(
          `${topicPrefix}/device/${id}/input/off`,
          turnDeviceOff.bind(null, id)
        );
        subscribe(
          `${topicPrefix}/device/${id}/input/active`,
          setActive.bind(null, id)
        );
      });
      subscribe(`${topicPrefix}/device/audio/input/power`, powerAudio);
